...
import { autoCorrelate, chooseLockedMelodyPitch, chooseMelodyPitch, detectChord, estimateKey, estimateMeter, frequencyToMidi, midiToNote, scaleForKey } from '../src/music.js';
...
test('locked melody never leaves detected key',()=>{const outsideChord={root:1,intervals:[0,4,7]};const key={root:0,mode:'major'};for(let i=0;i<100;i++){const midi=chooseLockedMelodyPitch(outsideChord,key,64,i%4===0);assert.ok([0,2,4,5,7,9,11].includes(midi%12));}});
